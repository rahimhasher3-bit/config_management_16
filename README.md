...
На четвертом этапе реализована работа основных команд оболочки с VFS.

Реализованы команды:

- `ls` — вывод содержимого текущего каталога;
- `cd` — переход между каталогами VFS;
- `date` — вывод текущей даты и времени;
- `cal` — вывод календаря.

Поддерживается переход в родительский каталог с помощью:
